...
assets/
  ...
  img/                originals + generated 480/768/1200px JPG + WebP
                        variants (see tools/build_responsive.py)
```
...
One system, driven by eight tokens at the top of `styles.css` — `--ease`,
`--reveal-dist`, `--reveal-dur`, `--stagger`, `--hover-dur`, `--ui-dur`, `--zoom`,
`--zoom-in`. Change the feel of the whole page there rather than section by section.
...
- Images carry `width`/`height` (no layout shift), are `lazy` below the fold,
  and the hero is preloaded with `imagesrcset`, so the preload matches the
  responsive pick instead of double-downloading. Every content photo ships as
  `<picture>` with a WebP `srcset` and a JPG fallback `srcset`: 480 / 768 /
  1200 px plus the original width, never upscaled, with `sizes` matching each
  slot. A phone loads the 480 px WebP (hero 15 KB instead of 191 KB, portrait
  12 KB instead of 135 KB); a desktop gets the full file. Variants are
  generated by `python tools/build_responsive.py` (Pillow, quality 72) —
  re-run it after swapping any photo. `picture { display: contents; }` keeps
  the wrapper from breaking the `height: 100%` chains, so no layout CSS had
  to change. Only the hero loads up front.
...
**Known next step:** AVIF variants would shave another ~20–30% off the WebP
files. Skipped deliberately — encoding is ~10× slower, the gain over WebP is
small, and every extra format multiplies the variant count. Revisit if image
weight ever dominates the budget again.
